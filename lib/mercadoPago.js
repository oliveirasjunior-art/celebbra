...
// Cria uma preferência de pagamento para um pedido com um ou mais itens.
// Retorna a URL (init_point) para redirecionar o cliente ao checkout.
async function criarPreferencia({ idReserva, itens, taxaEntrega, emailCliente }) {
  const preference = new Preference(client);

  const itemsMP = itens.map((it) => ({
    id: it.idItem,
    title: it.nome,
  ...
  if (taxaEntrega && Number(taxaEntrega) > 0) {
    itemsMP.push({
      id: "TAXA-ENTREGA",
      title: "Taxa de entrega e retirada",
      quantity: 1,
